...
def ensure_hex_color(color_value):
    """Convert color name or hex code to hex format for HexColor()"""
    if not color_value:
        return "#FFFFFF"
    # If it's already a hex code, return it
    if color_value.startswith("#"):
        return color_value
    # Otherwise, convert using css_color_to_hex
    try:
        return css_color_to_hex(color_value)
    except:
        return "#FFFFFF"
...
def render_schedule_pdf(
    ...
    *,
    ...
    for event in events:
        ...
        hex_color = ensure_hex_color(meta.get("calendar_color", "#FFFFFF"))
    ...
    if DRAW_ALL_DAY_BAND:
        ...
        if all_day_events:
            ...
            for idx, (st, en, title, meta) in enumerate(to_draw):
                ...
                if draw_shapes:
                    c.setFillColor(HexColor(ensure_hex_color(meta.get("calendar_color", "#FFFFFF"))))
